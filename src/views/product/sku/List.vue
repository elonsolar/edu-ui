    ...
    <a-card :bordered="false">
      ...
      >
        ...
        <span slot="category" slot-scope="text">
          <a-tag :color="text |categoryColorFilter"> {{ text | categoryFilter }}</a-tag>
        </span>
        ...
        <span slot="action" slot-scope="text, record">
          <template>
            <a-space>
              ...
              <a-popconfirm title="确认下架?" @confirm="() => takeDown(record)">
                  <a v-if="record.status==20081">下架</a>
              </a-popconfirm>
            </a-space>
          </template>
        </span>
...
import { listSku,updateSku,createSku,takeDownSku, placeUpSku } from '@/api/product'
...
export default {
  ...
  methods: {
    ...
    placeUp(record){
      ...
      placeUpSku({"id":record.id}).then(res => {
    ...
    takeDown(record){
      this.confirmLoading = true
      takeDownSku({"id":record.id}).then(res => {
          this.confirmLoading = false
          // 刷新表格
          this.$refs.table.refresh()
    ...
    handleOk () {
      ...
      form.validateFields((errors, values) => {
        if (!errors) {
          ...
          if (values.id > 0) {
            // 修改 e.g.
            updateSku(values.id, values).then(res => {
              this.visible = false
              this.confirmLoading = false
              // 重置表单数据
              form.resetFields()
    ...
    changeStatus(status,record){
      ...
      updateSku(record.id,record).then(res=>{
