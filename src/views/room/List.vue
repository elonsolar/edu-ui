          ...
          <a-row :gutter="48">
            ...
            <!-- <a-col :md="8" :sm="24">
              <a-form-item label="类型">
                <a-select v-model="queryParam.status" placeholder="请选择" default-value=0>
                  <a-select-option value=0>混用</a-select-option>
                  <a-select-option value=1>绘画</a-select-option>
                  <a-select-option value=1>书法</a-select-option>
                </a-select>
              </a-form-item>
            </a-col> -->
...
import { listRoom, createRoom, updateRoom } from '@/api/room'
...
export default {
  ...
  methods: {
    ...
    handleOk () {
      ...
      form.validateFields((errors, values) => {
        if (!errors) {
          ...
          if (values.id > 0) {
            // 修改 e.g.
            updateRoom(values.id, values).then(res => {
              this.visible = false
    ...
    changeStatus(status,record){
      ...
      updateRoom(record.id,record).then(res=>{
