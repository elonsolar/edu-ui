        ...
        <span slot="action" slot-scope="text, record">
          <template>
            <a-space size="small">
                ...
                <a v-action:setPermission @click="handlePermission(record)">权限设置</a>
            </a-space>
          </template>
        </span>
      </s-table>
...
import { listTenant,updateTenant,createTenant } from '@/api/tenant'

import CreateForm from './modules/CreateForm'
import AuthPermission from './modules/AuthPermission'
...
export default {
  ...
  methods: {
    ...
    handleOk () {
      ...
      form.validateFields((errors, values) => {
        if (!errors) {
          ...
          if (values.id > 0) {
            // 修改 e.g.
            updateTenant(values.id, values).then(res => {
              this.visible = false
    ...
    changeStatus(status,record){
      ...
      updateTenant(record.id,record).then(res=>{
