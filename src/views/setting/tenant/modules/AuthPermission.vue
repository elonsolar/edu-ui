...
import {  saveTenantPermission,getTenantPermission} from '@/api/tenant'
...
export default {
  ...
  data () {
    ...
    return {
      treeData: [{"id":1,"name":'xm',children:[{"id":2,"name":"xh"},{"id":3,"name":"xh"}]}],
      replaceFields: {
        title: 'name',
        key:'id'
      },
  ...
  methods:{
    getTree(){
      let that =this
      getPermissionTree({"parentId":0}).then(res=>{
            that.treeData=res.data
      })

      getTenantPermission({"tenantId":this.model.id}).then(res=>{
         that.selectedKeys.checked=res.permissions
      })
    },
    submit(){
      saveTenantPermission({"tenantId":this.model.id,"permissions":this.selectedKeys.checked}).then(res=>{
