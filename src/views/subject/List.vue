    ...
    <a-card :bordered="false">
            ...
            <a-col :md="8" :sm="24">
              <a-form-item label="类别">
                <a-select v-model="queryParam.category" placeholder="请选择" default-value=0>
                  <a-select-option value=0>绘画</a-select-option>
      ...
      >
        ...
        <span slot="category" slot-scope="text">
          <a-tag >{{ text | categoryFilter }}</a-tag>
        </span>
...
import { listSubject, createSubject, updateSubject } from '@/api/subject'
    ...
    handleOk () {
      ...
      form.validateFields((errors, values) => {
        if (!errors) {
          ...
          if (values.id > 0) {
            // 修改 e.g.
            updateSubject(values.id, values).then(res => {
              this.visible = false
